...
function march_cell(
    ...
    iter = y -> let (τ, dU!dx, H, Cf, _, _, θ, H1, CE) = y
        Pim1, Dim1, transim1, sepim1 = qim1

        _, _, _, _, trans_th_im1, sep_th_im1, _, _, _ = yim1
        ...
        ynew = get_intermediary(
            [P, D, transim1, sepim1], Re, ue, due!dx;
            M∞ = M∞,
            r = r,
        ...
        _, _, _, _, trans_th, sep_th, _, _, _ = ynew
...
function march_around(
    ...
    (
        ...
        Cp = (@. 1.0 - ue ^ 2),
        drag = drag,
        transition_flag = fetch_i(3),
        separation_flag = fetch_i(4),
        transition_threshold = fetch_i(9),
        separation_threshold = fetch_i(10),
    )
