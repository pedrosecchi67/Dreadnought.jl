...
function get_intermediary(
    ...
    H1 = D * ρ * qe / (P + sqrt(eps(typeof(P))))
    H1 = clamp(H1, 3.1, 12.0)
    ...
    Reθ0 = Drela.Reθ0(Hl)
    dn!dReθ = Drela.dn!dReθ(Hl) * (1.0 - trans) + Drela.turbulent_dn!dReθ * trans
...
function march_cell(
    ...
    iter = y -> let (τ, dU!dx, H, Cf, _, _, θ, H1, CE, Reθ, Reθ0, dn!dReθ) = y
        Pim1, Dim1, Nim1, transim1, sepim1 = qim1

        _, _, _, _, trans_th_im1, sep_th_im1, _, _, _, Reθim1, Reθ0im1, _ = yim1
        ...
        ynew = get_intermediary(
            [P, D, Nim1, transim1, sepim1], Re, ue, due!dx;
            M∞ = M∞,
            Ncr = Ncr,
        ...
        _, _, _, _, trans_th, sep_th, _, _, _, Reθ, Reθ0, _ = ynew
        ...
        crit = max(
            trans,
            ponderation(Reθim1 - Reθ0im1, Reθ - Reθ0)
        )

        N = max(
            Nim1,
            Nim1 + dn!dReθ * (Reθ - Reθim1) * crit
        )
...
function march_around(
    ...
    tups_neg = [
        begin
            q, y, drag = march_cell(
                ...
                M∞ = M∞, Re = Re,
                Ncr = Ncr, H_separation = H_separation,
                γ = 1.4, 
                n_iter = n_iter,
                ω = ω,
    ...
    (
        ...
        Cp = (@. 1.0 - ue ^ 2),
        drag = drag,
        transition_flag = fetch_i(4),
        separation_flag = fetch_i(5),
        transition_threshold = fetch_i(10),
        separation_threshold = fetch_i(11),
    )
